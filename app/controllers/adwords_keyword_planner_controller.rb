...
class AdwordsKeywordPlannerController < ApplicationController
  def connect
  ...
  def connect_account
    token = Token.find_by(code_type: KeywordPlanner::CODE_TYPE)
    token.update_attributes(sub: params[:customer_id])
    render json: { status: 200, message: 'Account added successfully' }
  end

  def disconnect
    token = Token.find_by(code_type: KeywordPlanner::CODE_TYPE)
    if token.destroy
      render json: { status: 200, message: 'Disconnected successfully' }
    else
      render json: {
        status: 401, message: token.errors.full_messages.to_sentence
      }
    end
  end

  def keyword_planner_token
    token = Token.find_by(code_type: KeywordPlanner::CODE_TYPE)
