...
var Utility = {
  validateURL: function(url) {
    ...
    toastr.error('Please check the Domain, It should be a valid domain');
    return false;
  },
  ...
  closeModal: function($scope, $mdDialog) {
    return angular.extend($scope.$new(), {
      close: function() {
        $mdDialog.cancel();
      }
    });
  ...
  toggle_leftbar: function() {
    ...
    setTimeout(function() {
      // wait 500ms before calling resize
  ...
  rightbarTopPos: function() {
    ...
    if ($('body').hasClass('infobar-overlay')) {
      if (
        $('body>header, body.horizontal-nav>#wrapper>header').hasClass(
          'navbar-fixed-top'
        )
      ) {
  ...
  rightbarRightPos: function() {
    ...
    if ($('body').hasClass('layout-boxed')) {
      ...
      $('.infobar-active.infobar-overlay .infobar-wrapper').css(
        'right',
        ending_right
      );
    ...
    plugin.init = function() {
      ...
      $('body').on('click', 'ul.acc-menu a', function() {
        ...
        if (
          ...
          $(this).parents('ul.acc-menu').length > 1
        ) {
      ...
      while (true) {
        if ($(targetAnchor).hasClass('sub-li')) {
          $(targetAnchor)
            .parent('li.li-block')
            .addClass('active');
          parent.addClass('active');
        } else {
          $('.acc-menu li.active').removeClass('active');
...
$(document).ready(function() {
  ...
  // ------------------------------
  $('body').on('click', '.yamm .dropdown-menu, .dropdown-menu-form', function(
    e
  ) {
    e.stopPropagation();
  });
