class CreateKeywordDailyRankingsMatView < ActiveRecord::Migration[4.2]
  def up
      execute <<~SQL
      ...
      add_index :keyword_daily_rankings_view, [:date, :keyword_id, :business_id], 
        # unique: true,
        name: :keyword_daily_rankings_view_cmp_index
      add_index :keyword_daily_rankings_view, :business_id
  end

  def down
    execute("DROP MATERIALIZED VIEW IF EXISTS keyword_daily_rankings_view")
  end
