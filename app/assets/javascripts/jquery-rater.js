    ...
    function init() {
        elems.each( function() {
            ...
            if(opts.mode == MODE_INPUT && ($this.prop("tagName") == 'INPUT' || $this.prop("tagName") == 'SELECT') ){
              ...
              var outer = $('<div class="raterater-input"></div>').attr('data-id', iId).attr('data-rater',$this.val()).data('input',$this);
              $this.attr('data-id', iId).attr('data-id', iId).attr('data-rater',$this.val()).data('input',$this).after(outer).hide();
              object = $this = outer;
            }
    ...
    function initializePositions() {
        elems.each( function() {
          ...
          if(opts.mode == MODE_INPUT)
             ...
             */
            var rating = parseFloat( $this.attr( 'data-rater' ) );

            if(isNaN(rating)){
                console.log("rating is not real number")
            }
            rating = rating || opts.rating;
