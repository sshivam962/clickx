class SuperAdmin::DocumentsController < ApplicationController
  layout 'base'
  before_action :perform_authorization
  before_action :set_document, only: %i[edit update destroy]

  def index
    @documents = Document.agency_documents.includes(:document_attachment, :thumbnail)
                         .where(search_query)
                         .order(:title)
  ...
  def create
    ...
    if @document.save
      ...
      redirect_to super_admin_documents_path
    else
      flash[:error] = @document.errors.full_messages.to_sentence
      redirect_to new_super_admin_document_path(@document)
    end
  end
  ...
  def update
    if @document.update(document_params)
      ...
      flash[:error] = @document.errors.full_messages.to_sentence
      redirect_to edit_super_admin_document_path(@document)
    end
  end
  ...
  def document_params
    params.require(:document).permit(
      :title, :description, :category, :document_link,
